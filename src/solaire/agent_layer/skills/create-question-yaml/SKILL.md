...
metadata:
  ...
  version: "1.1"
  label: 按规范创建题目
  tool_patterns: "bank.create_item bank.update_item bank.get_item bank.search_items file.read memory.* agent.*"
  suggested_user_input: 请帮我按题库规范创建一道新题，并确保格式校验通过。
---

## 工作流程

1. 按字段组织单题正文（字段清单与样例见同目录 `references/yaml-template.md`）
...
- `references/yaml-template.md`：可复制粘贴的 YAML 骨架
