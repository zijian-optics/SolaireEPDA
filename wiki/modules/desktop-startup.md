...
- **握手门槛**：Rust 在 `RunEvent::Ready` 后的后台线程里，对 `http://127.0.0.1:8000/api/health` 轮询最多 **120 秒**（默认，可用 `SOLAIRE_BACKEND_HEALTH_WAIT_SECS` 覆盖），且要求 JSON 中 `status == "ok"` 且 `product == "sol_edu"`。其它业务接口返回 200 **不能替代**该检查。
